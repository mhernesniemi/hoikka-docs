...
The shared `Db` type deliberately excludes `transaction`: it would type-check on better-sqlite3, but D1 has no interactive transactions, so it would blow up in production. Use `atomic()` instead.

## Atomic writes

`atomic()` (also in `index.ts`) runs several write statements atomically on both targets: D1 executes `batch()` as one implicit transaction; better-sqlite3 wraps the statements in `BEGIN`/`COMMIT`. Pass unexecuted Drizzle statements (don't await them yourself).

```typescript
import { db, atomic } from "$lib/server/db";

await atomic([
	db.update(orders).set({ state: "cancelled" }).where(eq(orders.id, orderId)),
	db.update(productVariants).set({ stock: sql`stock + 1` }).where(eq(productVariants.id, variantId))
]);
```
...
Migrations live in `drizzle/` and are generated from the schema. On the node target they apply automatically on first database use; on Cloudflare apply them explicitly with Wrangler.
