...
`/account/*` is protected by a layout guard (`src/routes/(storefront)/account/+layout.server.ts`) — requires a logged-in user with a verified email and a customer record. Users with an unverified email are redirected to `/verify-email`; everyone else missing a requirement goes to `/sign-in`.
