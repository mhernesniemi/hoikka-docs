...
- **Database** (`src/lib/server/db/index.ts`) — `db` is a lazy `Proxy`. On node it resolves to a `better-sqlite3` singleton (`node.ts`, migrates on first use). On Cloudflare it resolves to a Drizzle client over the D1 binding (`platform.env.DB`), cached per request on `event.locals`. Multi-statement writes go through the `atomic()` helper: `batch()` on D1, `BEGIN`/`COMMIT` on better-sqlite3. See [Database](/core/database#atomic-writes).
...
```
Request → hooks.server.ts (demo guard, edge cache, session + customer sync)
    ↓
