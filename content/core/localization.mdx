...
import {
  ...
  TRANSLATION_LANGUAGES,  // [{ code: "fi", name: "Finnish" }] — excludes default
...
A single service (`src/lib/server/services/translations.ts`) handles all 8 entities with a consistent API:
...
await translationService.getAssetTranslations(assetId);
await translationService.getAllAssetTranslations(assetIds); // Bulk: given assets
await translationService.upsertAssetTranslation(assetId, "fi", { alt });
```
...
Products, variants, facets, collections, and assets use a shared `TranslationEditor` component (`src/lib/components/admin/TranslationEditor.svelte`) that renders a separate "Translations" card with language tabs. Inactive tabs use a `hidden` class to keep all fields mounted (important for rich text editors), and the component uses the HTML `form` attribute to associate its inputs with the main form.

### Inline Language Tabs

Content pages render language tabs directly in the main form card instead, with the same hidden-tab mounting.
...
Currently, the storefront serves the default language only — fields like `product.name` come directly from the entity tables. Translation tables store non-default language content managed through the admin UI.

To add multi-language storefront support, you would detect the language (from URL prefix, cookie, or `Accept-Language` header) and merge the matching translation into the response at the service or route level.
